...
-- Ajout des colonnes manquantes à la table users si elles n'existent pas
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'subscription_plan') THEN
    ALTER TABLE users ADD COLUMN subscription_plan TEXT DEFAULT 'gratuit';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'subscription_start') THEN
...
COMMENT ON COLUMN users.subscription_plan IS 'Plan actuel: gratuit, premium, conseil';
COMMENT ON COLUMN users.subscription_status IS 'Statut: inactive, active, cancelled, past_due';
COMMENT ON COLUMN users.subscription_start IS 'Date de début de l abonnement';
COMMENT ON COLUMN users.subscription_end IS 'Date de fin de l abonnement';
COMMENT ON COLUMN users.stripe_customer_id IS 'ID du customer Stripe associé';
